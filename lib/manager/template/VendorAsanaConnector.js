...
class VendorAsanaConnector extends AsanaConnector {
  onCreate(app) {
    ...
    app.get('/:vendorUserId/me', this.lookupUser(), async (req, res) => {
      ...
      console.log(`me refresh_token: ${JSON.stringify(userContext.vendorToken)}`);
    ...
    app.get('/:vendorUserId/start', this.lookupUser(), async (req, res) => {
      ...
      try {
        ...
        // Save the new userContext with the webhook configuration added.
        await this.saveUser(fusebitContext, webhook.userContext);
      } catch (e) {
