...
class AsanaConnector extends OAuthConnector {
  ...
  onCreate(app) {
    ...
    app.post('/:vendorUserId/webhook/:webhookId', this.lookupUser(), this.httpWebhook());
  }
  ...
  async createNewWebhook(fusebitContext, userContext, asana, resourceId, options) {
    ...
    const target = `${fusebitContext.baseUrl}/${userContext.vendorUserId}/webhook/${webhookId}`;
   ...
   */
  async deleteWebhook(userContext, asana, webhookId) {
    const userProfile = userContext.vendorUserProfile;
   ...
   * @param {*} asana An Asana client created through createClient.
   */
  async deleteAllWebhooks(userContext, client) {
    await Promise.all(
      Object.keys(userContext.vendorUserProfile.webhooks).map(async (webhookId) => {
        ...
        try {
          await this.deleteWebhook(userContext, client, webhookId);
  ...
  async onHttpWebhook(req, res) {
    ...
    try {
      if (this.onWebhookEvent) {
        await this.onWebhookEvent(userProfile, webhookId, req.body.events);
      }
    } catch (e) {
   ...
   */
  async onWebhookEvent(userProfile, webhookId, events) {
    Sdk.debug(`Executing default webhook handler`);
  }

  /**
   * Asana does not correctly reflect the refresh_token back when refreshing the access token.
   */
  async refreshAccessToken(fusebitContext, tokenContext, redirectUri) {
    const currentRefreshToken = tokenContext.refresh_token;
    const result = await super.refreshAccessToken(fusebitContext, tokenContext, redirectUri);
